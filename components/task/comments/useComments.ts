...
export const useComments = function (taskId: String) {
	...
	const { mutate: removeResultMutation, isPending: removingResult } =
		useMutation({
			...
			mutationFn: () =>
				DB.updateDocument(
					DB_ID,
					COLLECTION_COMMENTS,
					resultCommentID.value as string,
					{
						taskResult: false,
					}
				),
	...
	const { mutate: updateMutation, isPending: updating } = useMutation({
		...
		mutationFn: () =>
			DB.updateDocument(DB_ID, COLLECTION_COMMENTS, commentToEditID.value, {
	...
	const { mutate: deleteMutation } = useMutation({
		...
		mutationFn: () =>
			DB.deleteDocument(DB_ID, COLLECTION_COMMENTS, commentToDel.value),
		onSuccess: () => {
