...
const {
	...
	edit,
	addNew,
} = useGroupEditForm(selectedGroup, props.completeFields)
...
<template>
	<UForm :state="{}" @submit="handleSubmit" class="text-qwe-50">
		<UFormGroup required :label="text.labels.name" name="company">
			...
			<UIAppearMessage
				:condition="errors.company !== undefined && errors.company.length > 0"
		...
		<UFormGroup required :label="text.labels.link" name="link">
			...
			<UIAppearMessage
				:condition="errors.link !== undefined && errors.link.length > 0"
				:message="errors.link"
			/>
		</UFormGroup>
		<UButton
			...
			type="submit"
			class="w-1/4 justify-center mx-auto"
			>{{ btnText }}</UButton
		>
	</UForm>
</template>
